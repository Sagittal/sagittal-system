import { computeIrrationalDecimalFromScaledVector, Decimal, Extrema } from "@sagittal/general"
...
describe("secondary comma zones", (): void => {
    it("they check out", (): void => {
        ...
        expected.forEach((extrema: Extrema<{ of: { decimal: Decimal } }>, index: number): void => {
            expect(computeIrrationalDecimalFromScaledVector(actual[index].extrema[0]!)).toBeCloseTo(
                extrema[0].decimal,
            )
            expect(computeIrrationalDecimalFromScaledVector(actual[index].extrema[1]!)).toBeCloseTo(
                extrema[1].decimal,
            )
        })
    })
